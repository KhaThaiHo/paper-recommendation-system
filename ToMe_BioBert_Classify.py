...
PIPELINE OVERVIEW:
  1. Preprocess: Title + Abstract + Keywords → combined text
    2. Tokenize: text → token embeddings (via BioBERT)
...
class BioBertClassifier(nn.Module):
    def __init__(self, num_labels: int, use_tome: bool = False, tome_r: int = 8):
        ...
        if use_tome:
            ...
            print("[ToMe OFF] Standard BioBERT (no merging)")
...
def run_benchmark(
    ...
    - Baseline BioBERT (no ToMe)
    - BioBERT + Token Merging
    ...
    for use_tome in [False, True]:
        ...
        model = BioBertClassifier(num_labels, use_tome=use_tome, tome_r=tome_r).to(device)
...
if __name__ == "__main__":
    ...
    baseline, tome = run_benchmark(
        df,
        num_epochs=2,     # ↑ for real training
        batch_size=8,
        max_length=256,   # ↑ to 256/512 for full abstracts
